...
// 2. Static class всегда наследуется от Оbject
//      (Попытка наследования от чего либо другого приводит к ошибке компиляции).
// 3. Static class не реализует интерфейсы.
//      Попытка наследования от интерфейса приводит к ошибке уровня компиляции. 
// 4. Содержит только статические члены
//      (наличие в нем нестатического члена приведет к ошибке компиляции). 
...
internal static class StaticClass //: object  // Наследование только от Object (явно или неявно)
{
    ...
    static StaticClass() => Console.WriteLine("Static Constructor");
